...
public class ContestRepository implements IContestRepository {
    ...
    public boolean existsById(String id) {
        ...
        return contestMap.entrySet().stream().map(a -> a.getKey()).anyMatch(a->a.contains(id));
    }

    @Override
    public void delete(Contest entity) {
        // TODO Auto-generated method stub
        contestMap.entrySet().removeIf(a -> a.getValue().equals(entity)); 
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        return this.contestMap.values().stream().filter(x -> x.getLevel()==level).collect(Collectors.toList());
    }
